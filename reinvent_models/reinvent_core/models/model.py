...
from reinvent_models.reinvent_core.utils import collate_fn
...
class RNN(tnn.Module):
    ...
    def __init__(self, voc_size: int, layer_size: int = 512, num_layers: int = 3, cell_type: str = 'gru',
                 embedding_layer_size: int = 256, dropout: float = 0.0, layer_normalization: bool = False) -> None:
        """
    ...
    def forward(self, input_vector: torch.Tensor, hidden_state: Union[torch.Tensor, Sequence[torch.Tensor]] = None) \
...
class Model:
    ...
    def __init__(self, vocabulary: mv.Vocabulary, tokenizer: mv.SMILESTokenizer,
                 network_params: dict = None, max_sequence_length: int = 256,
                 no_cuda: bool = False):
        """
        ...
        :param no_cuda: whether to use cuda or not
        """
    ...
    def load_from_file(cls, file_path: str, sampling_mode: bool = False):
        ...
        :param file_path: input file path containing the model
        :param sampling_mode: whether to use sampling (inference, evaluation) or training mode
