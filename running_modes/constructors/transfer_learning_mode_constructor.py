...
from running_modes.transfer_learning.mol2mol_transfer_learning_runner import Mol2MolTransferLearningRunner
...
class TransferLearningModeConstructor:
    def __new__(cls, configuration: GeneralConfigurationEnvelope) -> BaseRunningMode:
        ...
        if cls._configuration.model_type == model_type.DEFAULT:
        ...
        elif cls._configuration.model_type == model_type.MOL2MOL:
            config = from_dict(data_class=Mol2MolTransferLearningConfiguration, data=cls._configuration.parameters)
            model_config = ModelConfiguration(model_type=cls._configuration.model_type, model_mode=model_mode.TRAINING,
            ...
            runner = Mol2MolTransferLearningRunner(configuration=config, model=model, logger=logger,
                                                      optimizer=optimizer)
        else:
