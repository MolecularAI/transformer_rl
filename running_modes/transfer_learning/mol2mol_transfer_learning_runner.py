...
from reinvent_models.mol2mol.dataset.preprocessing import get_pair_generator
import reinvent_models.mol2mol.dataset.paired_dataset as mol2mol_dataset
...
class Mol2MolTransferLearningRunner(BaseRunningMode):
    """Trains a given model."""

    def __init__(
        self,
        model: Model,
        configuration: Mol2MolTransferLearningConfiguration,
    ...
    def _initialize_dataloader(self, data, *, shuffle=None):
        ...
        if self._configuration.ranking_loss_penalty:
            ...
            dataset = mol2mol_dataset.StratifiedPairedDataset(**paired_dataset_args)
            collate_fn = mol2mol_dataset.StratifiedPairedDataset.collate_fn
        else:
            dataset = mol2mol_dataset.PairedDataset(**paired_dataset_args)
