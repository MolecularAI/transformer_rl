...
from running_modes.automated_curriculum_learning.production_strategy.mol2mol_production_strategy import \
    Mol2MolProductionStrategy
    ...
    def __new__(cls, prior: GenerativeModelBase, inception: Inception,
        ...
        if production_strategy_enum.STANDARD == configuration.name:
            ...
            production = LinkInventProductionStrategy(prior=prior, diversity_filter=diversity_filter,
                                                      ...
                                                      configuration=configuration, logger=logger)
            return production
        elif production_strategy_enum.MOL2MOL == configuration.name:
            diversity_filter = Mol2MolDiversityFilter(configuration.diversity_filter)
            production = Mol2MolProductionStrategy(prior=prior, diversity_filter=diversity_filter,
