...
from running_modes.configurations import GeneralConfigurationEnvelope, CreateModelConfiguration, \
    LinkInventCreateModelConfiguration, Mol2MolCreateModelConfiguration

from running_modes.create_model import CreateModelRunner, LinkInventCreateModelRunner, Mol2MolCreateModelRunner
...
class CreateModelModeConstructor:
    def __new__(cls, configuration: GeneralConfigurationEnvelope) -> BaseRunningMode:
        ...
        elif cls._configuration.model_type == model_type_enum.MOL2MOL:
            config = from_dict(data_class=Mol2MolCreateModelConfiguration, data=cls._configuration.parameters)
            runner = Mol2MolCreateModelRunner(configuration=config, logger=logger)
        else:
