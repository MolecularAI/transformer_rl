...
class FragmentReactionSliceEnumerator:
    ...
    def enumerate(self, molecule: Mol, cuts: int) -> List[FragmentedMolecule]:
        ...
        for cut in range(1, cuts + 1):
            if cut == 1:
                outcomes = self._reactions.slice_molecule_to_fragments(molecule, self._chemical_reactions)
                for outcome in outcomes:
                    fragment_pairs = outcome.reaction_outcomes
                    for pair in fragment_pairs:
                        for indx, _ in enumerate(pair):
                            ...
                            if sliced_mol.original_smiles == sliced_mol.reassembled_smiles:
                                ...
                                sliced_mol.reaction_smarts.append(outcome.reaction_smarts)
        ...
        # for sliced_mol in sliced_mols:
        #     print(sliced_mol.scaffold_smiles, sliced_mol.decorations_smiles,
        #       sliced_mol.reaction_smarts)
    ...
    def _scaffold_slicing(self, slice: FragmentedMolecule, cut: int) -> Set[FragmentedMolecule]:
        ...
        if slice.decorations_count() == cut - 1:
            outcomes = self._reactions.slice_molecule_to_fragments(slice.scaffold, self._chemical_reactions)
            for outcome in outcomes:
                fragment_pairs = outcome.reaction_outcomes
                for pair in fragment_pairs:
                    ...
                    if scaffold:
                        ...
                        sliced_mol.reaction_smarts.extend(slice.reaction_smarts)

                        if sliced_mol.original_smiles == sliced_mol.reassembled_smiles:
                            sliced_mol.reaction_smarts.append(outcome.reaction_smarts)
                            #print(cut, len(sliced_mol.reaction_smarts), sliced_mol.reaction_smarts)
