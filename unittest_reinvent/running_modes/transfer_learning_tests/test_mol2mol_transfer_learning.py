...
from running_modes.configurations.transfer_learning.mol2mol_transfer_learning_configuration import \
    Mol2MolTransferLearningConfiguration
...
from unittest_reinvent.fixtures.test_config import MAIN_TEST_PATH, MOL2MOL_PRIOR_PATH, MOL2MOL_SMILES_SET_PATH
...
class TestMol2MolTransferLearning(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.workfolder = os.path.join(MAIN_TEST_PATH, mt_enum.MOL2MOL + rm_enum.TRANSFER_LEARNING)
        ...
        self.parameters = Mol2MolTransferLearningConfiguration(input_model_path=MOL2MOL_PRIOR_PATH,
                                                                  output_model_path=os.path.join(self.workfolder,'model.chpt'),
                                                                  input_smiles_path=MOL2MOL_SMILES_SET_PATH,
                                                                  num_epochs=2,
                                                                  optimizer=self.opt_parameters,
                                                                  validation_percentage=0.1,
                                                                  pairs=pair_config,
                                                                  validation_seed=1234)
        self.general_config = GeneralConfigurationEnvelope(model_type=mt_enum.MOL2MOL, logging=vars(log_config),
