...
from reinvent_scoring.scoring.diversity_filters.mol2mol.diversity_filter import DiversityFilter as Mol2MolDiversityFilter
...
from running_modes.automated_curriculum_learning.curriculum_strategy.mol2mol_curriculum_strategy import \
    Mol2MolCurriculumStrategy
...
class CurriculumStrategy:
    def __new__(cls, prior: GenerativeModelBase, agent: GenerativeModelBase,
        ...
        if curriculum_strategy_enum.NO_CURRICULUM != configuration.name:
            if curriculum_strategy_enum.MOL2MOL != configuration.name:
        ...
        if curriculum_strategy_enum.STANDARD == configuration.name:
            ...
            cl_strategy = LinkInventCurriculumStrategy(prior=prior, agent=agent, configuration=configuration,
                                                       diversity_filter=diversity_filter, logger=logger, inception=inception)
            return cl_strategy
        elif curriculum_strategy_enum.MOL2MOL == configuration.name:
            diversity_filter = Mol2MolDiversityFilter(configuration.diversity_filter)
            cl_strategy = Mol2MolCurriculumStrategy(prior=prior, agent=agent, configuration=configuration,
